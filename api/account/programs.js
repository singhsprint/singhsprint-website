...
module.exports = async function handler(req, res) {
  ...
  // ------------------------------------------------------------
  // GET ?action=my-program — read-only member view, no admin gate.
  // (was /api/account/my-program; folded in here to stay under the
  // Vercel Hobby 12-function limit.)
  // ------------------------------------------------------------
  if (req.method === 'GET' && req.query?.action === 'my-program') {
